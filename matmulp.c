...
void thread_per_matrix_m(struct matrix *A, struct matrix *B) {
    pthread_t thread;
...
void thread_per_row_m(struct matrix *A, struct matrix *B) {
    int n = A->rows;
    pthread_t threads[n];
    int **res = (int **) calloc(A->rows, sizeof(int *));
    for (int i = 0; i < A->rows; i++) {
    ...
    write_matrix(1, C);
    for (int i = 0; i < C->rows; i++)        //to free heap 
        free(C->arr[i]);
    free(C->arr);
    free(C);
}
...
void thread_per_element_m(struct matrix *A, struct matrix *B) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 1 && argc != 4) {      // just name or name with 3 files names
    ...
    if (A->cols != B->rows) {
        ...
        for (int i = 0; i < A->rows; i++)                //if error free heaps
            free(A->arr[i]);
        free(A->arr);                           
        free(A);
